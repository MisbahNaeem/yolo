...
def detect(save_img=False):
    ...
    print(f"FPS = {frame_counter/elapsed_time:0.2f} fps")
        #frame_times.append(elapsed_time)
      ########
        # if len(frame_times) >= 1:  # Calculate FPS after processing 10 frames
        #   fps = 1 / (sum(frame_times) / len(frame_times))
        #   print(f'FPS: {fps:.2f}')
        #   frame_times = []  # Reset frame times for next 10 frames
